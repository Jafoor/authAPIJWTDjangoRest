...
const FormComponent = ({ params }: { params: { id: string } }) => {
  ...
  useEffect(() => {
    async function getCategory() {
      const data = await fetch(`${APP_URI}/api/blog-category`);
      const res = await data.json();
      setCategory(res);
    }

    async function getResource() {
      const data = await fetch(`${APP_URI}/api/blog/${id}`);
      const blog = await data.json();
      const res = blog.blogs;
      setFormData({
        ...formData,
        user: res.user || "",
        title: res.title || "",
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      if (formData.category && formData.title && formData.shortDescription) {
        ...
        const res = await fetch(`${APP_URI}/api/blog/${id}`, {
          method: "PUT",
          headers: {
