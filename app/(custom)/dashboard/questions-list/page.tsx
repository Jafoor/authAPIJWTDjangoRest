...
const Page = () => {
    ...
    useEffect(() => {
        ...
        async function getQuestions() {
            try {
                const data = await fetch(`${APP_URI}/api/question/${subTopic}/admin-all`);
                const res = await data.json();
                setQuestions(res);
              } catch (error) {
                console.log(error);
    ...
    <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        <tbody>
            { questions.map((item) => (
                ...
                <th scope="row" dangerouslySetInnerHTML={{ __html: item.question }} className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                </th>
                </Link>
